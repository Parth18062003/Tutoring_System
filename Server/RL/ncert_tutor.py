...
class NCERTStudentEnv(gym.Env):
        ...
        for subject, content in self.curriculum.SUBJECTS.items():
            if isinstance(content, list):
                for topic in content:
                    ...
                    difficulty = self.curriculum.TOPIC_DIFFICULTY.get(subject, {}).get(topic, 5.0)
                    topic_base_difficulty_map[full_name] = np.clip(difficulty / 10.0, 0.1, 0.9)
            else:
                for subsubject, subtopics in content.items():
                    for topic in subtopics:
                        full_name = f"{subject}-{subsubject}-{topic}"
                        self.topics.append(full_name)
                        difficulty = self.curriculum.TOPIC_DIFFICULTY.get(subject, {}).get(subsubject, {}).get(topic, 5.0)
                        topic_base_difficulty_map[full_name] = np.clip(difficulty / 10.0, 0.1, 0.9)
        self.num_topics = len(self.topics)
        if self.num_topics == 0: raise ValueError("No topics found.")
        self.topic_to_idx = {topic: idx for idx, topic in enumerate(self.topics)}
        self.topic_base_difficulty = np.array([topic_base_difficulty_map[topic] for topic in self.topics], dtype=np.float32)


    def _build_curriculum_graph(self):
        # (Implementation as before)
        self.prerequisite_matrix = np.zeros((self.num_topics, self.num_topics), dtype=np.float32)
        if not hasattr(self.curriculum, 'PREREQUISITES') or not self.curriculum.PREREQUISITES: return
        for target_tuple, prereq_list in self.curriculum.PREREQUISITES.items():
            target_key = "-".join(map(str, target_tuple)) if isinstance(target_tuple, tuple) else str(target_tuple)
            if target_key in self.topic_to_idx:
                target_idx = self.topic_to_idx[target_key]
                for prereq_tuple in prereq_list:
                    prereq_key = "-".join(map(str, prereq_tuple)) if isinstance(prereq_tuple, tuple) else str(prereq_tuple)
    ...
    def _create_student_profiles(self, num_students):
        ...
        for _ in range(num_students):
            profile = {
                'base_learning_rate': np.random.uniform(0.08, 0.30) * LEARNING_ACCELERATION, # Apply global acceleration
                ...
                'attention_decay': np.random.uniform(0.97, 0.998), # Slower decay
                ...
                'motivation_factors': {'intrinsic': np.random.uniform(0.5, 0.95), 'extrinsic_sensitivity': np.random.uniform(0.3, 0.8), 'persistence': np.random.uniform(0.98, 0.998), 'mastery_goal_orientation': np.random.uniform(0.3, 0.7)}, # Higher persistence & intrinsic start
    ...
    def _initialize_student_state(self, profile_idx=None):
        ...
        if profile_idx is None: profile_idx = np.random.randint(len(self.student_profiles))
    ...
    def _get_obs(self):
        # (Implementation as before)
        if self.current_student is None: raise RuntimeError("Reset env first.")
        obs = {
    ...
    def step(self, action: np.ndarray):
        if self.current_student is None: raise ValueError("Reset env first.")
        ...
        if topic_priorities.size > topic_idx and topic_priorities[topic_idx] < 0.3 and np.random.random() < override_prob:
            ...
            if len(high_priority_topics) > 0:
                ...
                self._debug_print(f"Override topic {topic_idx} (prio {topic_priorities[topic_idx]:.2f}) with {new_topic_idx} (prio {topic_priorities[new_topic_idx]:.2f})")
        ...
        self.current_student['strategy_history'] *= 0.85; self.current_student['strategy_history'][strategy_idx] += 0.15
        self.current_student['topic_attempts'][topic_idx] += 1
        last_topic_idx = self.current_student['current_topic_idx']
        self.current_student['steps_on_current_topic'][0] = self.current_student['steps_on_current_topic'][0] + 1 if topic_idx == last_topic_idx else 1
        self.current_student['current_topic_idx'] = topic_idx

        # --- Calculate Factors ---
        prereq_satisfaction = self._calculate_prerequisite_satisfaction(topic_idx)
        base_difficulty = self.topic_base_difficulty[topic_idx]
        difficulty_adjustment = {DifficultyLevel.EASIER: -0.2, DifficultyLevel.NORMAL: 0.0, DifficultyLevel.HARDER: 0.2}[difficulty_action]
        ...
        strategy_style_match = self._calculate_strategy_learning_style_match(strategy_idx)
        scaffolding_factor = 1.0
        if scaffolding_action != ScaffoldingLevel.NONE:
            need = np.clip((1.0 - prev_mastery) * effective_difficulty, 0, 1)
            benefit = profile['scaffolding_benefit'] * (1.0 if scaffolding_action == ScaffoldingLevel.GUIDANCE else 0.6)
            scaffolding_factor = 1.0 + benefit * need
        length_factor = {ContentLength.CONCISE: 0.85, ContentLength.STANDARD: 1.0, ContentLength.DETAILED: 1.1}[length_action]

        # --- Update State ---
        # Cog Load (Increased sensitivity, reduced recovery)
        load_increase = effective_difficulty * length_factor * (1.0 - profile['cognitive_traits']['working_memory'] * 0.4) # Less WM benefit
        load_increase *= (1.0 - profile['scaffolding_benefit'] * 0.5 * (scaffolding_factor - 1.0))
        strategy_load_factor = {TeachingStrategies.EXPLANATION: 1.15, TeachingStrategies.PRACTICE: 1.0, TeachingStrategies.ASSESSMENT: 1.25, TeachingStrategies.EXPLORATION: 0.9}.get(strategy, 1.05) # Slightly higher base loads
        load_increase *= strategy_load_factor
        natural_recovery = 0.06 * (1.0 - prev_cog_load) # Further reduced recovery
        # Mitigation from high attention/motivation
        load_mitigation = 0.1 * (self.current_student['attention'][0] - 0.5) + 0.05 * (self.current_student['motivation'][0] - 0.5)
        final_load_change = load_increase * 0.30 - natural_recovery - max(0, load_mitigation) # Increased scaling, add mitigation
        new_cog_load = np.clip(prev_cog_load + final_load_change, 0.05, 0.98) # Allow slightly lower min load
        self.current_student['cognitive_load'][0] = new_cog_load

        # Attention
        attention_change = 0.0
        strategy_attention_factor = {TeachingStrategies.INTERACTIVE: 0.1, TeachingStrategies.GAMIFICATION: 0.12, TeachingStrategies.STORYTELLING: 0.08, TeachingStrategies.EXPLANATION: -0.03}.get(strategy, 0.05) # Slightly nerf explanation
        attention_change += strategy_attention_factor - (length_factor - 1.0) * 0.05 - (new_cog_load - 0.5) * 0.15 # Stronger cog load penalty
        new_attention = prev_attention * profile['attention_decay'] + attention_change
        self.current_student['attention'][0] = np.clip(new_attention, 0.1, profile['attention_span'])

        # Mastery Gain
        base_learn_rate = profile['base_learning_rate'] # Already accelerated in profile creation
        ...
        learning_efficacy = (
            ...
            np.clip(1.0 - new_cog_load * 0.7, 0.15, 1.0) * # Slightly higher floor, reduced cog load impact
            self.current_student['motivation'][0]
        )
        max_potential_gain = max(0.001, (1.0 - prev_mastery))
        mastery_gain = base_learn_rate * learning_efficacy * scaffolding_factor * length_factor * max_potential_gain

        # Simulated Performance
        noise = np.random.normal(0, 0.15)
        simulated_performance = np.clip(prev_mastery + mastery_gain * 0.8 + noise - effective_difficulty * 0.2, 0.0, 1.0)
        self.current_student['recent_performance'][0] = 0.7 * self.current_student['recent_performance'][0] + 0.3 * simulated_performance

        # Misconceptions
        misconception_cleared = False; misconception_formed = False
        if current_misconception > 0:
            clear_prob = 0.05 + 0.30 * learning_efficacy * profile['feedback_sensitivity'] # Increased base clear prob
            if feedback_action == FeedbackType.ELABORATED: clear_prob *= 1.3
            if strategy in [TeachingStrategies.EXPLANATION, TeachingStrategies.DEMONSTRATION]: clear_prob *= 1.1
            if np.random.random() < clear_prob:
                 reduction = np.random.uniform(0.6, 1.0) * current_misconception # Higher reduction chance
                 self.current_student['misconceptions'][topic_idx] = np.clip(current_misconception - reduction, 0, 1)
                 misconception_cleared = True
                 mastery_gain *= 1.15 # Slightly larger bonus

        form_risk = profile['misconception_propensity'] * (1.0 - prereq_satisfaction + effective_difficulty + (1.0 - simulated_performance)) * (1.0 - prev_mastery)
        if np.random.random() < np.clip(form_risk * 0.15, 0, 0.12) and not misconception_cleared: # Reduced overall risk slightly
             self.current_student['misconceptions'][topic_idx] = np.random.uniform(0.2, 0.6) # Slightly lower severity
        ...
        self.current_student['mastery'][topic_idx] = np.clip(prev_mastery + final_mastery_gain, 0, 1)
        ...
        intrinsic_boost = 0.015 * mot_factors['intrinsic'] # Slightly increased intrinsic boost
        motivation_change += intrinsic_boost
        # Give direct boost for high mastery gain
        if final_mastery_gain > 0.05:
             motivation_change += 0.10 * mot_factors['mastery_goal_orientation'] # Boost based on goal orientation

        perceived_success = (simulated_performance * 0.4 + final_mastery_gain * 15.0 * 0.6) # Even more weight on gain
        success_threshold = 0.5; failure_threshold = 0.25
        if perceived_success > success_threshold:
            motivation_change += 0.08 * mot_factors['extrinsic_sensitivity']
             ...
             motivation_change -= 0.04 * (1.0 - mot_factors['mastery_goal_orientation']) # Further reduced penalty
        if misconception_formed: motivation_change -= 0.04
        if misconception_cleared: motivation_change += 0.08
        persistence_factor = mot_factors.get('persistence', 0.985) # Ensure high persistence
        new_motivation = prev_motivation * persistence_factor + motivation_change
        self.current_student['motivation'][0] = np.clip(new_motivation, 0.20, 0.99) # Increased lower bound, allow higher max

        # Engagement (Add gain boost)
        eng_factors = profile['engagement_factors']
        engagement_change = 0.01 # Base slight increase
        if final_mastery_gain > 0.05: engagement_change += 0.05 # Boost from significant gain
        if perceived_success > 0.6: engagement_change += eng_factors['success_boost']
        elif perceived_success < 0.3: engagement_change -= eng_factors['failure_penalty'] * 0.8 # Reduced penalty
        strategy_freq = self.current_student['strategy_history'][strategy_idx]
        engagement_change += eng_factors['variety_seeking'] * (1.0 - strategy_freq) * 0.2
        engagement_change += eng_factors['challenge_seeking'] * (effective_difficulty - 0.5) * 0.1
        engagement_change -= (new_cog_load - 0.4) * 0.10 # Reduced cog load penalty
        engagement_change += eng_factors['interest_boost'] * 0.1
        new_engagement = prev_engagement * 0.96 + engagement_change # Slightly slower decay
        self.current_student['engagement'][0] = np.clip(new_engagement, 0.15, 0.98) # Higher lower bound
        ...
        reward = self._calculate_reward(
            ...
            new_cog_load, new_engagement, self.current_student['motivation'][0], # Pass motivation too
        ...
        if self.current_step % 25 == 0: # Print less frequently
            ...
            self._debug_print(f"  Action: Strat={strategy.name}, Topic={topic_idx}, Diff={difficulty_action.name}, Scaff={scaffolding_action.name}, Len={length_action.name}")
            self._debug_print(f"  State In: Prereq={prereq_satisfaction:.2f}, StyleMatch={strategy_style_match:.2f}, Attn={prev_attention:.2f}, CogLd={prev_cog_load:.2f}, Motiv={prev_motivation:.2f}, Mastery={prev_mastery:.3f}")
            self._debug_print(f"  Dynamics: EffDiff={effective_difficulty:.2f}, LrnEffic={learning_efficacy:.3f}, ScaffF={scaffolding_factor:.2f}, LenF={length_factor:.2f}")
            self._debug_print(f"  Outcome: MasteryGain={final_mastery_gain:.4f}, SimPerf={simulated_performance:.3f}, MisconF={misconception_formed}, MisconC={misconception_cleared}")
            self._debug_print(f"  State Out: Attn={self.current_student['attention'][0]:.2f}, CogLd={new_cog_load:.2f}, Motiv={self.current_student['motivation'][0]:.2f}, Eng={new_engagement:.2f}")
        ...
        info = {'action': action_int.tolist(), 'mastery_gain': final_mastery_gain, 'sim_performance': simulated_performance, 'engagement': new_engagement, 'cog_load': new_cog_load, 'attention': self.current_student['attention'][0], 'motivation': self.current_student['motivation'][0], 'eff_difficulty': effective_difficulty, 'prereq_sat': prereq_satisfaction, 'miscon_formed': misconception_formed, 'miscon_cleared': misconception_cleared, 'reward': reward}
        self.history.append(info)

        # Collect metrics at the end of the episode
        if done or truncated:
            self.episode_metrics = self.collect_episode_metrics()
            info['episode_metrics'] = self.episode_metrics # Add to final info dict

        return self._get_obs(), reward, done, truncated, info

    def _calculate_topic_priority(self):
        """Calculate priority scores for each topic (heuristic)."""
        if self.current_student is None: return np.zeros(self.num_topics) # Handle case before reset
        ...
        for i in range(self.num_topics):
            ...
            forgetting_risk = np.clip(time_since[i] / (self.max_steps*0.5), 0, 1) * (1 - mastery[i]**0.5) / (1 + attempts[i]*0.1)
            # Learning potential higher if ready and not mastered
            learning_potential = readiness * (1 - mastery[i])
            # Misconception priority
            misconception_factor = 1 + self.current_student['misconceptions'][i] * 0.5

            priorities[i] = (learning_potential * 0.6 + forgetting_risk * 0.3) * misconception_factor
    ...
    def _calculate_prerequisite_satisfaction(self, topic_idx):
        # (Implementation as before)
        if self.prerequisite_matrix is None or topic_idx >= self.prerequisite_matrix.shape[0]: return 1.0
        prereq_indices = np.where(self.prerequisite_matrix[topic_idx, :] > 0)[0]
        ...
        if len(valid_indices) == 0: return 1.0
    ...
    def _calculate_strategy_learning_style_match(self, strategy_idx):
        # (Implementation as before)
        style_match_matrix = np.array([[0.6,0.5,0.9,0.2],[0.9,0.6,0.5,0.7],[0.7,0.5,0.6,0.9],[0.6,0.4,0.5,0.9],[0.5,0.6,0.8,0.5],[0.5,0.9,0.6,0.6],[0.7,0.9,0.8,0.4],[0.8,0.7,0.6,0.9],[0.7,0.6,0.8,0.5]])
    ...
    def _apply_enhanced_forgetting(self, current_topic_idx):
        ...
        profile = self.current_student['profile']
        for i in range(self.num_topics):
            if i == current_topic_idx: continue
            ...
            repetitions = self.current_student['topic_attempts'][i] + 1 # Add 1 to avoid log(0)
            # Strength increases logarithmically with repetitions, influenced by base strength factor
            strength = profile['memory_strength_factor'] * np.log1p(repetitions) * 50 # Scaled strength

            # Simplified exponential decay based on time and strength
            retention_factor = np.exp(-t / max(10, strength)) # Avoid division by zero, base timescale
            ...
            self.current_student['mastery'][i] = np.clip(m - (m - target_mastery) * decay_rate, 0, 1)
    ...
    def _calculate_reward(self, mastery_gain, prev_mastery,
        ...
        reward += 50.0 * max(0, mastery_gain) # Primary driver, increased weight
        ...
        if motivation > 0.6: reward += (0.6 * (motivation - 0.6)) * state_reward_factor # Reward high motivation
        if engagement > 0.6: reward += (0.4 * (engagement - 0.6)) * state_reward_factor # Reward high engagement

        # --- Appropriateness Penalties (No Reduction Factor Anymore) ---
        # Difficulty Match (Zone of Proximal Development)
        target_difficulty = np.clip(0.2 + prev_mastery * 0.5, 0.1, 0.8) # Ideal difficulty target
        ...
        needed_scaffolding = prev_mastery < 0.45 and effective_difficulty > 0.55 # Adjusted thresholds
        ...
        if misconception_formed: reward -= 2.5 # Increased penalty
        if misconception_cleared: reward += 1.5 # Increased reward
        ...
        if mastery_gain < 0.005 and prev_mastery < 0.9 and steps_on_topic > 4:
            reward -= 0.10 * (steps_on_topic - 4) # Increased stagnation penalty

        # --- Exploration Bonus ---
        attempts = self.current_student['topic_attempts'][topic_idx]
        if attempts < 2: reward += 0.2 / (attempts + 1) # Slightly stronger initial exploration
        ...
        if cog_load < 0.2:
            ...
            reward -= 1.2 * (cog_load - 0.7)**2  # Keep penalizing excessive load

        # Tiny survival bonus
        reward += 0.001

        self._debug_print(f"DEBUG Reward Calc: Gain={mastery_gain:.4f} -> Reward={reward:.4f}")
        return reward
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            if not self.current_student: print("Env not init."); return
            ...
            if self.history:
                info=self.history[-1]; act=info['action']
                ...
                print(f"Action: {act_desc}"); print(f"  Outcome: EffDiff={info['eff_difficulty']:.2f},Prereq={info['prereq_sat']:.2f},Gain={info['mastery_gain']:.3f},SimPerf={info['sim_performance']:.2f},MisconF={info['miscon_formed']},MisconC={info['miscon_cleared']},Rew={info['reward']:.3f}")
            m_val = self.current_student['mastery'][idx] if 0<=idx<self.num_topics else -1
            att_val=int(self.current_student['topic_attempts'][idx]) if 0<=idx<self.num_topics else -1
            mis_val=self.current_student['misconceptions'][idx] if 0<=idx<self.num_topics else -1
            print(f"State: AvgM={np.mean(self.current_student['mastery']):.3f},Eng={self.current_student['engagement'][0]:.2f},Att={self.current_student['attention'][0]:.2f},CogLd={self.current_student['cognitive_load'][0]:.2f},Mot={self.current_student['motivation'][0]:.2f}")
            print(f"  Topic: M={m_val:.3f},Attmpt={att_val},StepsOn={int(self.current_student['steps_on_current_topic'][0])},Miscon={mis_val:.2f}")
        else: super().render(mode=mode)
    ...
    def collect_episode_metrics(self):
        ...
        metrics = {
            ...
            'final_misconceptions_count': int(np.sum(self.current_student['misconceptions'] > 0.1)), # Count non-trivial misconceptions
...
class FlattenObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env: NCERTStudentEnv):
        super().__init__(env)
        self.topics=getattr(env,'topics',[]); self.num_topics=getattr(env,'num_topics',0)
        self.topic_to_idx=getattr(env,'topic_to_idx',{}); self.topic_base_difficulty=getattr(env,'topic_base_difficulty',np.array([]))
        self.prerequisite_matrix=getattr(env,'prerequisite_matrix',np.array([[]]))
        flat_size=0; self._component_order=list(env.observation_space.spaces.keys()); self._component_shapes={}
        for key in self._component_order:
            space=env.observation_space.spaces[key]; size=np.prod(space.shape) if isinstance(space,spaces.Box) else 1
            flat_size+=int(size); self._component_shapes[key]=int(size)
        self.observation_space=spaces.Box(low=-np.inf,high=np.inf,shape=(flat_size,),dtype=np.float32)

    def observation(self, obs: Dict[str, Any]) -> np.ndarray:
        flat_obs_list=[]
        for key in self._component_order:
            comp=obs[key]
            if isinstance(comp,np.ndarray): flat_obs_list.append(comp.astype(np.float32).flatten())
            elif isinstance(comp,(int,float,np.number)):
                 norm_val=float(comp)/float(self.num_topics) if key=='current_topic_idx' and self.num_topics>0 else float(comp)
                 flat_obs_list.append(np.array([norm_val] if key=='current_topic_idx' else [comp],dtype=np.float32))
            else: raise TypeError(f"Unexpected type '{key}': {type(comp)}")
        final_obs=np.concatenate(flat_obs_list)
        if final_obs.shape[0]!=self.observation_space.shape[0]: raise ValueError(f"Shape mismatch: Exp {self.observation_space.shape[0]}, Got {final_obs.shape[0]}")
        return final_obs
...
class NCERTLearningSystem:
    ...
    def __init__(self, num_students=20, max_steps=250, log_dir="./ncert_tutor_logs_enhanced", num_cpu=4):
        self.num_students=num_students; self.max_steps=max_steps; self.log_dir=log_dir; self.num_cpu=max(1,num_cpu)
        os.makedirs(log_dir,exist_ok=True); os.makedirs(f"{log_dir}/models",exist_ok=True); os.makedirs(f"{log_dir}/tensorboard",exist_ok=True); os.makedirs(f"{log_dir}/eval_logs", exist_ok=True) # Ensure eval_logs exists
        env_fns=[self._make_env(i) for i in range(self.num_cpu)]
        self.vec_env=SubprocVecEnv(env_fns) if self.num_cpu > 1 else DummyVecEnv(env_fns)
        print(f"Using {'SubprocVecEnv' if self.num_cpu > 1 else 'DummyVecEnv'} with {self.num_cpu} process(es).")
        self.model=None

    def _make_env(self, rank: int, seed: int = 0):
        def _init():
            env=NCERTStudentEnv(num_students=self.num_students,max_steps=self.max_steps)
            ...
            env=Monitor(env,log_file,info_keywords=('reward', 'mastery_gain', 'engagement', 'cog_load', 'motivation', 'eff_difficulty', 'miscon_formed', 'miscon_cleared'))
                ...
                class EnvProxy:
                    def __init__(self, vec_env):
                        ...
                        try:
                            ...
                            self.topic_to_idx = vec_env.get_attr('topic_to_idx')[0]
                            self.topic_base_difficulty = vec_env.get_attr('topic_base_difficulty')[0]
                            self.prerequisite_matrix = vec_env.get_attr('prerequisite_matrix')[0]
    ...
    def create_model(self, policy="MlpPolicy", learning_rate=1e-4, gamma=0.99, verbose=1, ent_coef=0.01, **ppo_kwargs):
        default_policy_kwargs=dict(net_arch=dict(pi=[256, 256],vf=[256, 256])) # Consider [512, 256] if needed
        final_policy_kwargs={**default_policy_kwargs,**ppo_kwargs.pop('policy_kwargs',{})}
        self.model=PPO(policy,self.vec_env,verbose=verbose,tensorboard_log=f"{self.log_dir}/tensorboard/",learning_rate=learning_rate,gamma=gamma,n_steps=2048,batch_size=64,n_epochs=15,gae_lambda=0.95,clip_range=0.2,ent_coef=ent_coef,policy_kwargs=final_policy_kwargs,**ppo_kwargs)
    ...
    def train_model(self, total_timesteps=2_000_000, eval_freq=50000, save_freq=200000, n_eval_episodes=20):
        ...
        eval_env=self._make_env(rank=999)() # Use distinct rank for eval monitor
        eval_callback=EvalCallback(eval_env,best_model_save_path=f"{self.log_dir}/models/best",log_path=eval_log_path,eval_freq=max(eval_freq//self.num_cpu,1),n_eval_episodes=n_eval_episodes,deterministic=True,render=False)
        checkpoint_callback=CheckpointCallback(save_freq=max(save_freq//self.num_cpu,1),save_path=f"{self.log_dir}/models/checkpoints",name_prefix="ncert_tutor_enhanced")
        ...
        try:
            ...
            self.model.learn(total_timesteps=total_timesteps,callback=[eval_callback,checkpoint_callback],progress_bar=True, reset_num_timesteps=(self.model.num_timesteps == 0))
    ...
    def load_model(self, path):
        # (Implementation as before)
        if not os.path.exists(path): print(f"Error: Model path not found - {path}"); self.model=None; return None
        try: self.model=PPO.load(path,env=self.vec_env); print(f"Model loaded: {path}"); return self.model
        except Exception as e: print(f"Error loading model: {e}"); self.model=None; return None


    def evaluate_model(self, n_episodes=20, render=False):
        """Evaluate the trained model and collect detailed metrics."""
        if self.model is None: print("No model loaded."); return None
        ...
        for i in range(n_episodes):
            obs, _ = eval_env.reset(); done, truncated = False, False; total_r, steps = 0, 0
            while not (done or truncated):
                action, _ = self.model.predict(obs, deterministic=True)
                obs, r, done, truncated, info = eval_env.step(action); total_r += r; steps += 1
                if render: eval_env.render()

            all_rewards.append(total_r); all_lengths.append(steps);
            ...
            if ep_info and hasattr(eval_env, 'env') and hasattr(eval_env.env, 'env'):
                ...
                final_mastery = episode_end_metrics.get('final_avg_mastery', -1.0)
            ...
            print(f"Ep {i+1}: R={total_r:.2f}, Len={steps}, FinalM={final_mastery:.3f}")

        # Aggregate detailed metrics
        avg_final_m = np.mean([m for m in all_final_masteries if m >= 0]) if any(m >= 0 for m in all_final_masteries) else -1.0
        avg_detailed = {}
        if all_episode_metrics:
             keys_to_avg = ['avg_engagement', 'avg_motivation', 'avg_cog_load', 'final_misconceptions_count']
             for key in keys_to_avg:
                 valid_vals = [m.get(key) for m in all_episode_metrics if m.get(key) is not None]
                 if valid_vals:
...
if __name__ == "__main__":
    ...
    N_CPUS = max(1, os.cpu_count() - 1) if os.cpu_count() else 4 # Leave one CPU free
    EVAL_FREQ = 100_000 # Evaluate less frequently during long runs
    SAVE_FREQ = 250_000

    print("Initializing NCERTLearningSystem...")
    system = NCERTLearningSystem(num_students=20, max_steps=250, log_dir=LOG_DIR, num_cpu=N_CPUS)

    print("\nChecking environment...")
    try: check_env(system.unwrapped_env); print("Env check OK.")
    except Exception as e: print(f"Env check FAILED: {e}. Proceeding cautiously.")
    ...
    for phase_idx, phase in enumerate(training_phases):
        ...
        print(f"Target Steps: {phase['timesteps']}, LR: {phase['learning_rate']}, Entropy: {phase['ent_coef']}")

        # Create model for phase 1, update hyperparameters for subsequent phases
        if phase_idx == 0:
            system.create_model(learning_rate=phase['learning_rate'], ent_coef=phase['ent_coef'])
        else:
            if system.model is None: # Load previous phase's best if starting mid-way or after interruption
                 last_best = os.path.join(LOG_DIR, "models", "best", "best_model.zip")
                 print(f"Loading model from previous phase: {last_best}")
                 if not system.load_model(last_best):
                      print("ERROR: Cannot load previous model to continue training. Exiting.")
                      exit()
            # Update hyperparameters for the current phase
            print(f"Updating model LR to {phase['learning_rate']} and EntCoef to {phase['ent_coef']}")
        ...
        print(f"End of Phase {phase_idx+1}. Model checkpoint saved to {phase_model_path}.zip")

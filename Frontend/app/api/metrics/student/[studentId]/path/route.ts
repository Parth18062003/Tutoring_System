...
export async function GET(
  ...
  try {
    const backendResponse = await fetch(`${API_BASE_URL}/metrics/student/${requestedStudentId}/path?page=${page}&limit=${limit}`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        ...(INTERNAL_API_SECRET && { 'X-Internal-Api-Secret': INTERNAL_API_SECRET }),
        'X-Authenticated-User-Id': session.user.id, // Send authenticated user ID
      },
       cache: 'no-store', // Path data changes frequently
    });
    const data = await backendResponse.json();
    if (!backendResponse.ok) {
       return NextResponse.json({ message: data.detail || 'Failed to fetch learning path from backend' }, { status: backendResponse.status });
    }
    return NextResponse.json(data);
  } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Internal Server Error';
      const status = (error instanceof TypeError && error.message.includes('fetch failed')) ? 503 : 500;
      return NextResponse.json({ message: 'Failed to connect to metrics service (path)', details: errorMessage }, { status });
  }
}